...
constexpr int credits(std::uint32_t amount) {
	// Lua configuration values are unsigned; scale before narrowing to int.
	return static_cast<int>(amount / DIVISOR + (amount % DIVISOR != 0));
}
